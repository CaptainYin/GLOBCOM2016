function [solution,subNet_o] = vone_RI_lsp(requests,subNet)
...
v_deg_cpu= requests.cpu.*degree(requests.matrix);
[~,o_v] = sort(v_deg_cpu,'descend');
RI = NodeRank_RI(subNet,flag);
[~,o_s] = sort(RI,'descend');
for v_n = 1:v_node_num
    v_node = o_v(v_n);%index of node_mapping
    for s_n = 1:s_node_num
        s_node = o_s(s_n);
        if(flag(s_node) == 0 && cpu_capacity(s_node) >= cpu_requirement && s_degree(s_node) >= v_degree(v_node))
            node_mapping(v_node) = s_node;%updata subNet then update RI and sort again ,according to subnet ,flags 
            flag(s_node) = 1;
            %*******************************************
            cpu_capacity(s_node)=cpu_capacity(s_node)-cpu_requirement;
