function [solution,subNet_o] = la_lec_dsp_vone(VONR,subNet)
...
node_capacity = subNet.cpu;% get the capacity of each substrate node
...
[node_num_sub,order_nn] = sort(sub_node_num(listing),'descend');% sort the subgraphs
...
for sub_id_id = 1:length(order_nn)
    ...
    % precheck for degree
    if(~isempty(find((s_degree_sort(1:v_node_num) - v_degree_sort) < 0)))
        solution.state = 0;
        continue;
    end
    
    % node rank for the layered subgraph
    r_s = node_rank_lec_layered(node_capacity(original_nodes),layered_sub_topo);% get the local information on each node
    [r_s_sort,order_r_s] = sort(r_s,'descend');% sort the local information
    ...
    layered_sub_topo(find(layered_sub_topo == 0)) = Inf;
